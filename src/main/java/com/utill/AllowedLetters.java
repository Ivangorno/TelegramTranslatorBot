package com.utill;

import java.util.Collection;
import java.util.HashSet;
import java.util.Set;

public class AllowedLetters {

    public final static Set<Character> FRENCH_LETTERS = Set.of('a', 'b', 'c', 'd', 'f', 'g', 'h', 'j', 'k', 'l', 'm', 'n', 'p', 'q', 'r', 's', 't',
            'v', 'w', 'x', 'z', 'e', 'i', 'o', 'u', 'y', 'â', 'à', 'ç', 'é', 'ê', 'ë', 'è', 'ï', 'î', 'ô', 'û', 'ù');

    public final static Set<Character> ENGLISH_LETTERS = Set.of('a', 'b', 'c', 'd', 'f', 'g', 'h', 'j', 'k', 'l', 'm', 'n', 'p', 'q', 'r', 's', 't',
            'v', 'w', 'x', 'z', 'e', 'i', 'o', 'u', 'y');

    public static Set<Character> combineAllowedLetters(){
        Set<Character> combinedLetters = new HashSet<>();
        combinedLetters.addAll(FRENCH_LETTERS);
        combinedLetters.addAll(ENGLISH_LETTERS);

        return combinedLetters;
    }

}





